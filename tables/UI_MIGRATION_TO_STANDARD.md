...
Щільність сітки уточнено 2026-09-04: при масштабі 100% рядок має висоту 30px, заголовки — 32px, а текст клітинки — 15px. Масштабування змінює ці величини пропорційно й синхронізує геометрію кнопок вставлення.

Доповнено 2026-09-05: самих змінних `--cell-h`/`--head-h` було недостатньо. Успадкований `16px/normal` разом із `padding: 5px` тримав кожен `th` на 33px, а оскільки `height: var(--head-h)` діяв і на `th.row-header` усередині body, шапка рядка піднімала весь рядок до висоти шапки таблиці. Тепер типографіка `th` задана явно (`800 13px/1.2`, `padding: 0 5px`), а висота розведена: `#grid thead th` бере `--head-h`, `#grid tbody th.row-header` — `--cell-h`.
