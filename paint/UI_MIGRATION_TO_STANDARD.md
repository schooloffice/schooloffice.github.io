...
**Адаптивність виконано.** До `760px` rail стає горизонтальною смугою внизу, а properties panel — drawer поверх полотна: закритий за замовчуванням, відкривається кнопкою «Панель» (`#panelToggleBtn`) і не звужує canvas. Drawer лежить у межах `.editor-body` і закінчується над смугою інструментів (`--rail-h-mobile`), тож смуга й рядок стану лишаються доступними. Перевіряє `tests/responsive-smoke.html`; контракт зафіксовано в `paint/UI_STANDARD.md`.
...
- **Selection/clipboard debt:** за потреби винести логіку виділення, copy/cut/paste/delete з `app.js` в окремий service-модуль, якщо вона знову почне розростатися.
