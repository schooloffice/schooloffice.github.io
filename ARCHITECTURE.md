...
Offline readiness рахується окремо для `text`, `tables`, `slides`, `paint`, `vector` і `flowcharts`: редактор готовий лише тоді, коли повні його власна група та `core`. Загальний статус головної сторінки готовий лише після успішної перевірки всіх шести редакторів. Інсталяція використовує незалежні settled-спроби, тому один недоступний файл не приховує стан решти груп, а повторне кешування можна запустити без очищення вже збережених ресурсів. Оновлення не забирає офлайн-готовність: якщо нова версія `sw.js` не докешувала ресурси редактора, готового в попередній версії (за її збереженим статусом або, якщо запис загубився чи застарів після аварійного завершення браузера, за вмістом її групових кешів), встановлення відхиляється, попередня версія лишається активною зі своїми кешами, а браузер повторить оновлення під час наступного відкриття.
