...
ПЛЮС Текст має об'єднання й розділення клітинок через координатну grid-модель, пошук із заміною, міжрядковий інтервал і номери сторінок (з колонтитулом у `.docx`). ПЛЮС Вектор має множинне виділення, групи та згладжені криві.

ПЛЮС Таблиці читають і пишуть `.xlsx` без сторонніх бібліотек (`tables/js/xlsx-*.js`): значення, формули, числові формати, стилі, ширини колонок, кілька аркушів і умовне форматування. Імпорт чужих книг розгортає протягнуті формули й чесно звітує про те, що не перенеслося. Наступний фокус — розширення бібліотеки функцій (пошук/підстановка, текстові) і точкова діаграма.
...
- `PROJECT_DIRECTION.md` — продуктова стратегія, рішення щодо редакторів, залежностей, ризиків і дорожньої карти
- `LICENSE` — MIT-ліцензія власного коду проєкту
- `THIRD_PARTY_NOTICES.md` — інвентаризація локальних сторонніх bundle-файлів
...
3. Винести зовнішні CDN-ресурси в локальний `vendor/` (зроблено): реліз постачає один переглянутий набір файлів, і це передумова офлайну.
...
`tests/browser-smoke.html` можна відкрити в браузері як додатковий smoke-тест DOM-структури, а `tests/run-browser-smoke.ps1` автоматизує цей сценарій через headless Chrome і додатково запускає поведінкові перевірки для Text, Flowcharts, Slides, Paint, Tables і Vector. Для `slides/` `slides/js/runtime.js` лишається тонкою module-entry обгорткою для стабільного підключення в HTML, а `tests/slides-behavior.html` перевіряє, що `SlidesApp.boot`, список слайдів, сцена і project helpers справді працюють у браузері. Зовнішні CDN-ресурси винесено в локальний `vendor/`; статичний аудит попереджає, якщо в HTML редактора знову зʼявиться зовнішній ресурс. Директорії `tests/.browser-profile*` і файли `.browser-smoke.*` є локальними артефактами запуску; вони ігноруються git і чистяться через `tests\cleanup-test-artifacts.ps1`.
...
**Робочий файл** — це той, у якому робота продовжується наступного разу: він повертає документ таким, яким його зберегли. **Обмін** — формати для передавання результату; вони описують не все, що вміє редактор, і редактор про це попереджає.

| Редактор | Робочий файл | Відкриває | Експорт |
|---|---|---|---|
| Текст | `.tekst` | `.tekst`, `.docx`, `.rtf`, `.txt` | `.docx`, `.rtf`, `.txt`, друк |
| Таблиці | `.tablytsia` | `.tablytsia`, `.xlsx`, `.csv` | `.xlsx`, `.csv`, друк |
| Слайди | `.slaydy` | `.slaydy` | `.pptx`, `.pdf`, друк |
| Малюнки | `.malyunok` | `.malyunok`, зображення | `.png`, `.jpg` |
| Вектор | `.vektor` | `.vektor` | `.svg`, `.png` |
| Схеми | `.shema` | `.shema` | `.png`, друк |

Файли, збережені до перейменування пакета (`.arttab`, `.artslides.json`, безлике `.json`), відкриваються як раніше.

Окремо від файлів кожен редактор веде **браузерну чернетку** — страховку від аварії вкладки чи живлення. Вона живе лише в цьому браузері, не замінює файл і при відкритті пропонується явно: за одним комп’ютером працює багато учнів.
...
- механізм браузерної чернетки спільний (`office-draft-storage.js`); редактор вирішує лише, що зберігати й коли пропонувати відновлення;
