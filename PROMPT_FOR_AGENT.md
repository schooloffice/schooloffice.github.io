...
- Це локальний пакет редакторів для шкільної інформатики: обробка документів відбувається в браузері, без сервера й акаунтів. Офлайн-запуск — мета продукту, тимчасово відкладена (див. розділ «Офлайн» у `PROJECT_DIRECTION.md`).
...
- Нову залежність додавати лише після перевірки ліцензії, роботи без зовнішніх запитів, bundle size і плану підтримки; постачати її локально у `vendor/` — CDN закритий, бо ламає майбутній офлайн.
- Новий runtime-файл додавати до тестового контракту. У `sw.js` нічого не додавати: поки офлайн відкладено, це перехідний worker, який знімає кеш попередніх релізів. Список precache буде згенеровано, коли офлайн повернеться.
