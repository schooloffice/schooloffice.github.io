'use strict';
/* core/draft-storage.js — сховище чернетки Тексту.
 *
 * Чернетка лежить в IndexedDB (structured clone, без ~5 МБ ліміту
 * localStorage — документ із вбудованими зображеннями легко його
 * перевищує). Якщо IDB недоступний або зависає (приватний режим; headless
 ...
 *
 * Це ТРЕТЯ копія одного механізму (paint → vector → text). Вона свідома:
 * винесення спільного сховища зачіпає три робочі редактори, а його помилка
 * означала б тиху втрату чернеток — тож це окремий крок із власною
 * перевіркою, а не побічний ефект появи чернетки в Тексті.
 */

const ArtDraftStorage = (() => {
  const DB_NAME = 'plus_text';
  const DB_VERSION = 1;
  const STORE = 'drafts';
  const DRAFT_KEY = 'current';
  const LS_KEY = 'plus_text_draft';
