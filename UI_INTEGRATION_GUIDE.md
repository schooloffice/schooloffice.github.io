...
Після підтвердження сторінка до перезавантаження нічого не зберігає. Слухачі `office-storage.js` на `pagehide`, `beforeunload` і `visibilitychange` спрацьовують раніше за редакторські й зупиняють їх, тож збереження під час вивантаження не записує документ поверх очищення, а питання «Покинути сайт?» не з'являється. Тому `office-storage.js` має підключатися раніше за скрипти редактора.

Очищення лишає в `localStorage` позначку `office_draft_cleared_v1:<key>`. Сторінка, відкрита до чужого очищення, більше не записує цю чернетку й показує статус «Не вдалося зберегти — завантажте файл».

Після збереженої роботи й 10 хвилин бездіяльності видима сторінка редактора один раз показує немодальну підказку про завантаження файла й завершення сеансу. Нічого при цьому не видаляється; для тестів затримку задає `?office-idle-hint-ms=`.
